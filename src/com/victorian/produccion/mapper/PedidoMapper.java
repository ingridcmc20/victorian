...
public interface PedidoMapper {
	@Select("SELECT idpedido, tipo_prenda,pr.descripcion as des_tipo_prenda, cantidad_prenda, modo_pago, fechapedido, "
			...
			+ "FROM t_pedido p inner join t_producto pr on pr.id_producto=p.tipo_prenda "
			+ "inner join t_estado e on e.id_estado=p.estadopedido "
			+ "inner join t_tipoconfeccion tc on tc.idtipoconfeccion=p.idtipoconfeccion "
			+ "inner join t_cliente c on c.idcliente=p.idcliente "
			+ "inner join t_modo_pago m on p.modo_pago=m.id_modo_pago "
	...
	@Select("SELECT idpedido, tipo_prenda,pr.descripcion as des_tipo_prenda, cantidad_prenda, modo_pago, fechapedido, "
			+ "fechaentrega, estadopedido, e.descripcion as des_estado, p.idtipoconfeccion, tc.descripcion as des_tipoconfeccion,"
			+ "c.nombre_cliente, m.descripcion as des_modo_pago, "
			+ "(select count(idfichatecnica)>0 from t_fichatecnica where idproducto=tipo_prenda) bool_ficha_tecnica "
			+ "FROM t_pedido p inner join t_producto pr on pr.id_producto=p.tipo_prenda "
			+ "inner join t_estado e on e.id_estado=p.estadopedido "
			+ "inner join t_tipoconfeccion tc on tc.idtipoconfeccion=p.idtipoconfeccion "
			+ "inner join t_cliente c on c.idcliente=p.idcliente "
	...
	@Insert("INSERT INTO t_pedido(tipo_prenda, cantidad_prenda, fechapedido, fechaentrega, estadopedido, idtipoconfeccion) "
	...
	@Select("SELECT distinct p.idpedido, p.tipo_prenda, p.cantidad_prenda, p.modo_pago, p.fechapedido, p.fechaentrega, p.estadopedido, p.idtipoconfeccion, p.idcliente "
			+ "FROM t_pedido p inner join t_plan_pedido pp on pp.idpedido=p.idpedido where idplan=#{idplan}")
