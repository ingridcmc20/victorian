...
public interface PedidoMapper {
	@Select("SELECT id_pedido, tipo_prenda,pr.descripcion as des_tipo_prenda, cantidad_prenda, modo_pago, fecha_registro as fechapedido, "
	...
	@Select("SELECT distinct p.id_pedido, p.tipo_prenda, p.cantidad_prenda, p.modo_pago, p.fecha_registro as fechapedido, p.fecha_entrega, p.id_estado, p.id_tipoconfeccion, p.idcliente "
			+ "FROM victorian.t_pedido p inner join victorian.t_plan_pedido pp on pp.id_pedido=p.id_pedido where idplan=#{idplan}")
	...
	@Select("SELECT id_pedido, tipo_prenda,pr.descripcion as des_tipo_prenda, cantidad_prenda, modo_pago, fecha_registro as fechapedido, "
			...
			+ "inner join victorian.t_cliente c on c.idcliente=p.idcliente "
			+ "inner join victorian.t_modo_pago m on p.modo_pago=m.id_modo_pago "
			+ "WHERE id_pedido=#{id_pedido} and p.id_estado=#{id_estado} "
			+ "order by c.nombre_cliente, fecha_registro")
