...
public interface OperarioMapper {
	...
	@Select("SELECT o.id_operario, o.id_tipooperario, u.apellido_paterno || ' ' || u.apellido_materno || ' ' "
			+ "|| u.nombre as nombre_operario,o.puntaje_acumulado FROM victorian.t_operario o inner join victorian.t_usuario u on u.id_usuario=o.id_operario "
	...
	@Select("SELECT o.id_operario, o.id_tipooperario, u.apellido_paterno || ' ' || u.apellido_materno || ' ' "
			+ "|| u.nombre as nombre_operario,o.puntaje_acumulado FROM victorian.t_operario o inner join victorian.t_usuario u on u.id_usuario=o.id_operario "
			+ "where id_tipooperario=#{id_tipooperario} and o.id_operario not in (select x.id_operario from victorian.t_ordentrabajo_operario x inner join victorian.t_orden_trabajo y on y.id_ordentrabajo=x.id_ordentrabajo where y.id_estado not in (6))")
	...
	@Select("SELECT o.id_operario||'' as id_operario "
			+ "FROM victorian.t_operario o inner join victorian.t_usuario u on u.id_usuario=o.id_operario inner join victorian.t_ordentrabajo_operario oto on oto.id_operario=o.id_operario "
