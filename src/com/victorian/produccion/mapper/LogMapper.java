...
public interface LogMapper {
	 ...
	 * @throws Exception
	 */
	@Insert("insert into victorian.t_log (idusuario, cod_menu,accion,descripcion,ip_client,host_client,fecha,hora,anio) " +
	 ...
	 **/
	@Select ("select distinct extract(year from fecha) from victorian.t_log")
	...
	@Select("SELECT a.pk_log_id,a.accion, a.descripcion, a.ip_client, a.fecha, a.hora, a.anio,a.ip_server_img, b.login , c.nombre as nombre_opMenu , d.nombre_sistema as modulo "+
			  "FROM  victorian.t_log a LEFT OUTER JOIN victorian.t_usuario b on (a.idusuario = b.idusuario) "+
			  "LEFT OUTER JOIN victorian.t_opcion_menu c on (a.cod_menu = c.cod_menu) "+
			  "LEFT OUTER JOIN victorian.t_sistema d on (c.cod_sistema = d.cod_sistema) "+
	...
	@Select("SELECT a.pk_log_id,a.accion, a.descripcion, a.ip_client, a.fecha, a.hora, a.anio,a.ip_server_img, b.login , c.nombre as nombre_opMenu , d.nombre_sistema as modulo "
			+ " FROM  victorian.t_log a LEFT OUTER JOIN victorian.t_usuario b on (a.idusuario = b.idusuario) "
			+ " LEFT OUTER JOIN victorian.t_opcion_menu c on (a.cod_menu = c.cod_menu) "
			+ " LEFT OUTER JOIN victorian.t_sistema d on (c.cod_sistema = d.cod_sistema) WHERE a.fecha  BETWEEN #{p_fecha_inicio} and #{p_fecha_fin}")
