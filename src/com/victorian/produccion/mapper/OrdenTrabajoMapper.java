...
public interface OrdenTrabajoMapper {
	...
	@Select("SELECT ot.id_orden_trabajo, ot.fecha_registro,cl.nombre_cliente,pr.descripcion as des_tipo_prenda,"+ 
				...
				"FROM t_orden_trabajo ot inner join t_pedido p on p.idpedido=ot.id_pedido "+
				"inner join t_cliente cl on cl.idcliente=p.idcliente "+
				"inner join t_producto pr on pr.id_producto=p.tipo_prenda "+
				"inner join t_tipoconfeccion tc on tc.idtipoconfeccion=p.idtipoconfeccion "+
				"inner join t_prioridad pri on pri.id_prioridad=ot.id_prioridad "+
				"inner join t_etapa e on e.id_etapa=ot.id_etapa "+
				"inner join t_estado es on es.id_estado=ot.id_estado "+
				"inner join t_orden_trabajo_detalle otd on otd.id_orden_trabajo=ot.id_orden_trabajo and otd.id_etapa=ot.id_etapa "+
				"inner join t_orden_trabajo_operario oto on oto.id_orden_trabajo=ot.id_orden_trabajo "+
	...
	@Update("UPDATE t_orden_trabajo SET id_estado=#{id_estado}, id_etapa=#{id_etapa} WHERE id_orden_trabajo=#{id_orden_trabajo}")
	public void updateEstadoYEtapa(OrdenTrabajo ordenTrabajo) throws Exception;
	
	@Update("UPDATE t_orden_trabajo SET id_estado=#{id_estado} WHERE id_orden_trabajo=#{id_orden_trabajo}")
	public void updateEstado(OrdenTrabajo ordenTrabajo) throws Exception;
	
	@Update("UPDATE t_orden_trabajo SET id_prioridad=#{id_prioridad} WHERE id_orden_trabajo=#{id_orden_trabajo}")
