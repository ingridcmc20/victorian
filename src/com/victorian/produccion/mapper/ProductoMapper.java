...
public interface ProductoMapper {
	...
	// No permite eliminar un Producto Asociado a un Negocio devuelve la
	// cantidad de Negocio asociado
	@Select("select count(1) from victorian.t_producto where id_negocio =  #{id_negocio}")
	...
	public List<Producto> buscarProductoByNegocioByProducto(@Param("p_id_negocio") Integer id_negocio,
			@Param("p_idproducto_padre") Integer id_producto_padre) throws Exception;

	@Select("select producto.*, producto.codigo_banco as codigoBanco from victorian.t_producto producto where codigo_banco = #{codigo_banco} and id_negocio = #{id_negocio}")
	public Producto buscarProductoByCodigoBanco(@Param("codigo_banco") String codigo_banco,
			@Param("id_negocio") Integer id_negocio) throws Exception;
