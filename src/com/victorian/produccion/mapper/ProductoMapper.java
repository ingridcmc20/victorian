...
public interface ProductoMapper {
	...
	@Select("select e.*, e.codigo_banco as codigo_banco, e.id_negocio from t_producto e where e.id_producto = #{p_producto}")
	public Producto findById(@Param("p_producto") Integer codigoProducto) throws Exception;
	
	//asociar producto y negocio
	@Select("select pr.id_producto, pr.idformato, pr.estado, pr.descripcion, pr.codigo_banco as codigoBanco from t_producto pr order by descripcion asc")
	public List<Producto> findAll() throws Exception;	
	
	@Select("select * from t_producto where estado = 'TRUE' order by descripcion asc")
	public List<Producto> listaProductosActivo() throws Exception;
	
	@Select("select * from t_producto where id_negocio =  #{p_negocioId} and estado = 'TRUE' order by descripcion")//
	public List<Producto> listaProductosPorNegocio(@Param("p_negocioId") Integer negocioId) throws Exception;
	
	@Insert("insert into t_producto (id_negocio, descripcion, estado, codigo_banco) values (#{id_negocio},#{descripcion},#{estado}, #{codigoBanco})")
	public void crearProducto(Producto producto) throws Exception;
	
	@Update("update t_producto set id_negocio = #{id_negocio}, idformato = #{idformato}, descripcion = #{descripcion}, estado = #{estado}, codigo_banco=#{codigoBanco} where id_producto= #{id_producto}")
	...
	@Select("select * from t_producto where id_producto = #{id_producto}")
	...
	@Select("select * from t_producto where id_negocio=#{id_negocio}")
	public List<Producto>buscarProductoByNegocio(@Param("id_negocio") Integer id_negocio)throws Exception;
	
	@Select("select * from t_producto where id_negocio=#{id_negocio} and id_producto = idproducto_padre")
	public List<Producto>buscarProductoProgramacion(@Param("id_negocio") Integer id_negocio)throws Exception;
	
	@Select("select * from t_producto where id_negocio=#{p_id_negocio} and idproducto_padre = #{p_idproducto_padre} order by descripcion")
	...
	@Select("select t_producto.*, t_producto.codigo_banco as codigoBanco from t_producto where codigo_banco = #{codigo_banco} and id_negocio = #{id_negocio}" )
	...
	@Select("select distinct p.* from t_tablero_comisiones tc inner join t_producto p on tc.id_producto = p.id_producto where tc.id_negocio = #{id_negocio} " )
	public List<Producto> listProductoDistinctTablComision(@Param("id_negocio") Integer id_negocio) throws Exception;

	@Select("select  * from t_producto where id_producto  = idproducto_padre and id_negocio = #{p_negocioId} ")//
